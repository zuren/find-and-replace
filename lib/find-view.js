...
class FindView {
    ...
    return (
      $.div({tabIndex: -1, className: 'find-and-replace'},
        $.header({className: 'header'},
          ...
          $.span({className: 'header-item options-label pull-right'},
            ...
            $.span({className: 'btn-group btn-toggle btn-group-options'},
              ...
              $.button({ref: 'scriptOptionButton', className: 'btn'},
                $.svg({className: "icon", innerHTML: '<use href="#find-and-replace-icon-script" />'})
              ),
        ...
        $.svg({style: {display: 'none'}, innerHTML: `
          ...
          <symbol id="find-and-replace-icon-script" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
            <rect x="3" y="6" width="2" height="8" rx="1" transform="translate(8.00000, 0.00000) rotate(45.000000)"></rect>
            <rect x="3" y="6.75" width="2" height="8" rx="1" transform="translate(-6.750000, 0.00000) rotate(-45.000000)"></rect>
  ...
  didShow() {
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      ...
      atom.tooltips.add(this.refs.scriptOptionButton, {
        title: "Use Script",
        keyBindingCommand: 'find-and-replace:toggle-script-option',
        keyBindingTarget: this.findEditor.element
      }),
  ...
  handleEvents() {
    ...
    this.subscriptions.add(this.model.getFindOptions().onDidChangeUseScript(this.updateSyntaxHighlighting.bind(this)));
    ...
    this.refs.scriptOptionButton.addEventListener('click', this.toggleScriptOption.bind(this));
  ...
  updateSyntaxHighlighting() {
    ...
    if (this.model.getFindOptions().useScript) {
  ...
  updateOptionButtons() {
    ...
    this.setOptionButtonState(this.refs.scriptOptionButton, this.model.getFindOptions().useScript);
  ...
  toggleScriptOption() {
    const useRegex = this.model.getFindOptions().useScript === false
      ? true
      : this.model.getFindOptions().useScript;

    this.search({useRegex: useRegex, useScript: !this.model.getFindOptions().useScript});
    if (!this.anyMarkersAreSelected()) {
      this.selectFirstMarkerAfterCursor();
    }
  }
