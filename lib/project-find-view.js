...
class ProjectFindView {
        ...
        $.header({className: 'header'},
          ...
          $.span({className: 'header-item options-label pull-right'},
            ...
            $.span({className: 'btn-group btn-toggle btn-group-options'},
              ...
              // $.button({ref: 'scriptOptionButton', className: 'btn option-script'},
              //   $.svg({className: "icon", innerHTML: `<use xlink:href="#find-and-replace-icon-script" />`})
              // ),
  ...
  didShow() {
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      ...
      //   title: "Use Script",
      //   keyBindingCommand: 'project-find:toggle-script-option',
      //   keyBindingTarget: this.findEditor.element
      // }),
  ...
  handleEvents() {
    ...
    // this.refs.scriptOptionButton.addEventListener('click', () => this.toggleScriptOption());
  ...
  toggleScriptOption() {
    this.search({onlyRunIfActive: true, useScript: !this.model.getFindOptions().useScript});
  }
