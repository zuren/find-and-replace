  ...
  replace(markers, replacePattern) {
    ...
    this.editor.transact(() => {
      ...
      for (let i = 0, n = markers.length; i < n; i++) {
        ...
        if (findRegex) replacementText = this.editor.getTextInBufferRange(bufferRange).replace(findRegex, replacePattern);
        if (this.findOptions.useScript) {
          const text = this.editor.getTextInBufferRange(bufferRange)
          text.replace(findRegex, (...args) => {
