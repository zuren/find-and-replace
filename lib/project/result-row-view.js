...
class MatchRowView {
  constructor({rowData, groupData, isSelected, replacePattern, regex, script}) {
    const props = {rowData, groupData, isSelected, replacePattern, regex, script};
    const previewData = {matches: rowData.matches, replacePattern, regex, script};
  ...
  update({rowData, groupData, isSelected, replacePattern, regex, script}) {
    const props = {rowData, groupData, isSelected, replacePattern, regex, script};
    const previewData = {matches: rowData.matches, replacePattern, regex, script};
  ...
  generatePreviewNode({matches, replacePattern, regex, script}) {
    const subnodes = [];
    ...
    for (const match of matches) {
      ...
      let replacementText = replacePattern || '';
      if (regex) replacementText = match.matchText.replace(regex, replacePattern);
      if (script) {
        match.matchText.replace(regex, (...args) => {
          const matches = args.slice(1, args.length - 2);
          const matchVars = args.reduce((acc, v, i) => ({ ...acc, [`$${i}`]: v }), {});
          replacementText = new Function("v", "with (v) { return (`" + replacePattern + "`)}")(matchVars);
        });
      }
